...
public class BookRepositoryImpl implements BookRepository {
	...
	public List<Book> getBookListByCategory(String category) {
		String sql = """
				select b_bookId, b_name,
				 b_unitPrice, b_author,
				  b_description, b_publisher,
				   b_category, b_unitsInStock,
	...
	public Set<Book> getBookListByFilter(Map<String, List<String>> filter) {
		...
		if (booksByFilter.contains("publisher")) {
			for (int j = 0; j < filter.get("publisher").size(); j++) {
				...
				String sql = """
						SELECT b_bookId, b_name, b_unitPrice,
						 b_author, b_description,
						  b_publisher, b_category,
						   b_unitsInStock, b_releaseDate,
						    b_condition
						     FROM book where
						      b_publisher LIKE '%' || ? || '%'
						""";
	...
	public Book getBookById(String bookId) {
		...
		if (rowCount != 0) {
			sql = "SELECT b_bookId, b_name, b_unitPrice, b_author, b_description,"
					+ " b_publisher, b_category, b_unitsInStock, b_releaseDate, b_condition"
					+ " FROM book where b_bookId=?";
